...
CREATED_VPC_ID=$(aws ec2 create-vpc --cidr-block 10.0.0.0/16 --query Vpc.VpcId --output text) && echo -e "VPC is successfully created" || echo "Failed creating"
...
CREATED_SUBNET_ID=$(aws ec2 create-subnet --vpc-id $CREATED_VPC_ID --cidr-block 10.0.1.0/24 --query Subnet.SubnetId --output text) && echo "Subnet is successfully created" || echo "Failed creating $(aws ec2 delete-vpc --vpc-id $CREATED_VPC_ID)"
SUBNET_TAGS=$(aws ec2 create-tags --resources $CREATED_SUBNET_ID --tags Key=Name,Value=Subnet-VPCid-$CREATED_VPC_ID)

echo "Creating Internet Gateway...."
#Create Internet Gateway

CREATED_INTERNET_GATEWAY_ID=$(aws ec2 create-internet-gateway --query InternetGateway.InternetGatewayId --output text) && echo "InternetGateway is successfully created" || echo "Failed creating $(aws ec2 delete-vpc --vpc-id $CREATED_VPC_ID)"
...
CREATED_ROUTE_TABLE_ID=$(aws ec2 create-route-table --vpc-id $CREATED_VPC_ID --query RouteTable.RouteTableId --output text) && echo "Route Table is successfully created" || echo "Failed creating $(aws ec2 delete-vpc --vpc-id $CREATED_VPC_ID)"
